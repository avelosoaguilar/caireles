<meta http-equiv="Content-Type" content="text/html; charset=utf-8" /> 
<div class="logoCairelesFooter">
	<a href="<%= request.getContextPath() %>/index.jsp">
		<img alt="Logo Caireles" src="<%= request.getContextPath() %>/img/logoFooter.png">	
	</a>
</div>

<div class="fraseComunicate">
	<p>Comunícate con nosotros e infórmate más sobre nuestra empresa y los productos y servicios que ofrecemos.</p>
</div>

<div class="contenidoColumnas">
	<div class="columnaEnlaces">
		<div class="contenidoColumnaEnlaces">
			<div class="tituloColumnaFooter">
				<p>Enlaces</p>
			</div>
			<div class="opcionesColumnaEnlaces">
				<p><a href="<%= request.getContextPath() %>/index.jsp">Inicio</a></p>
				<p><a href="<%= request.getContextPath() %>/jsp/quienesSomos.jsp">Quiénes somos</a></p>
				<p><a href="<%= request.getContextPath() %>/jsp/nuestrosServicios.jsp">Nuestros servicios</a></p>
				<p><a href="<%= request.getContextPath() %>/jsp/eventos.jsp">Eventos</a></p>
				<p><a href="<%= request.getContextPath() %>/jsp/contacto.jsp">Contacto</a></p>
			</div>
		</div>
	</div><!--
	
	--><div class="columnaContacto">
		<div class="contenidoColumnaContacto">
			<div class="tituloColumnaFooter">
				<p>Contacto</p>
			</div>
			<div class="infoColumnaContacto">
				<p>Caireles</p>
				<p>C/Van Dyck 21 - 37005 Salamanca</p>
				<div class="redesSocialesFooter">
					<div class="iconoRedSocialFooter">
						<a href="https://www.facebook.com/cairelessalamanca">
							<i class="fa fa-facebook"></i>
						</a>
					</div>
					<div class="iconoRedSocialFooter">
						<a href="https://twitter.com/caireles_sal">
							<i class="fa fa-twitter"></i>
						</a>
					</div>
					<div class="iconoRedSocialFooter">
						<a href="https://www.instagram.com/caireles_salamanca/">
							<i class="fa fa-instagram"></i>
						</a>
					</div>
					<div class="iconoRedSocialFooter">
						<a href="https://www.youtube.com/channel/UCJI5EAvHA0D8T-q_XZFbCFw">
							<i class="fa fa-youtube-play"></i>
						</a>
					</div>
					<div class="iconoRedSocialFooter">
						<a href="#">
							<i class="fa fa-envelope"></i>
						</a>
					</div>
				</div>
				<p>caireles@cairelessalamanca.com</p>
				<p>Teléfono: +34 923 249 168</p>
			</div>
		</div>
	</div><!--
	
	--><div class="columnaLegal">
		<div class="contenidoColumnaLegal">
			<div class="tituloColumnaFooter">
				<p>Información legal</p>
			</div>
			<div class="enlacesColumnaLegal">
				<p>Política de privacidad</p>
				<p>Política de cookies</p>
				<p>&nbsp</p>
				<p>&nbsp</p>
				<p>Diseño web: Caireles</p>
			</div>
		</div>		
	</div>
</div>

<div class="copyright">
	<p>Copyright 2019 © Caireles Salamanca. Todos los derechos reservados.</p>
</div>